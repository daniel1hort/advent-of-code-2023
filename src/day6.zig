...
fn parseInt(source: []const u8) u64 {
    var number: u64 = 0;
    for (source) |byte| {
...
pub fn part1() !void {
    var file = try std.fs.cwd().openFile("input/day6.in", .{});
    defer file.close();
    ...
    var line = stream.getWritten();
    var time_it = std.mem.split(u8, line[11 .. line.len - 1], " ");
    var index: u32 = 0;
    while (time_it.next()) |token| {
    ...
    var distance_it = std.mem.split(u8, line[11 .. line.len - 1], " ");
    index = 0;
    while (distance_it.next()) |token| {
        if (std.fmt.parseInt(u32, token, 10)) |number| {
    ...
    std.debug.print("day6 part1 answer = {d}\n", .{ways});
}
...
pub fn part2() !void {
    ...
    std.debug.print("day6 part2 answer = {d}\n", .{ways});
}
